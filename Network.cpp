...
string GetNetworkError() {
  ...
  if(!found) {
    char ret[256];
    std::snprintf(ret, sizeof(ret), "WSA Error code %d", err);
    return string(ret);
  } else {
...
bool Kangaroo::HandleRequest(TH_PARAM *p) {
  ...
  while( p->isRunning ) {
    ...
    case SERVER_RESETDEAD: {
      ...
      std::snprintf(response, sizeof(response), "OK\n");
...
void Kangaroo::AcceptConnections(SOCKET server_soc) {
  ...
  while(true) {
    ...
    if((clientSock = accept(server_soc,(struct sockaddr*)&client_add,&len)) < 0) {
      ...
      char info[256];
      std::snprintf(info, sizeof(info), "%s:%d", inet_ntoa(client_add.sin_addr), ntohs(client_add.sin_port));
#ifdef WIN64
