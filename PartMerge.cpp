...
string Kangaroo::GetPartName(std::string& partName,int i,bool tmpPart) {
  ...
  char tmp[256];
  if(tmpPart)
    std::snprintf(tmp, sizeof(tmp), "part%03d.tmp", i);
  else
    std::snprintf(tmp, sizeof(tmp), "part%03d", i);
...
bool Kangaroo::FillEmptyPartFromFile(std::string& partName,std::string& fileName,[[maybe_unused]] bool printStat) {
