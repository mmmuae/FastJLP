...
#define __noinline__
using gpu_uint64 = ulong;
using gpu_int64 = long;
using gpu_uint32 = uint;
using gpu_int32 = int;
#define GPU_THREAD_PTR(T) thread T*
#define GPU_CONST_THREAD_PTR(T) const thread T*
#else
#include <cstdint>
using gpu_uint64 = uint64_t;
using gpu_int64 = int64_t;
using gpu_uint32 = uint32_t;
using gpu_int32 = int32_t;
#define GPU_THREAD_PTR(T) T*
#define GPU_CONST_THREAD_PTR(T) const T*
...
#define GPU_U64_PTR GPU_THREAD_PTR(gpu_uint64)
#define GPU_CONST_U64_PTR GPU_CONST_THREAD_PTR(gpu_uint64)
#define GPU_I64_PTR GPU_THREAD_PTR(gpu_int64)
#define GPU_CONST_I64_PTR GPU_CONST_THREAD_PTR(gpu_int64)
#define GPU_I32_PTR GPU_THREAD_PTR(gpu_int32)
#define GPU_CONST_I32_PTR GPU_CONST_THREAD_PTR(gpu_int32)
...
#ifndef __METAL_VERSION__
#ifdef USE_SYMMETRY
__device__ void LoadKangaroos(GPU_U64_PTR a,uint64_t px[GPU_GRP_SIZE][4],uint64_t py[GPU_GRP_SIZE][4],uint64_t dist[GPU_GRP_SIZE][2],GPU_U64_PTR jumps) {
#else
__device__ void LoadKangaroos(GPU_U64_PTR  a,uint64_t px[GPU_GRP_SIZE][4],uint64_t py[GPU_GRP_SIZE][4],uint64_t dist[GPU_GRP_SIZE][2]) {
#endif
...
__device__ void LoadDists(GPU_U64_PTR a,uint64_t dist[GPU_GRP_SIZE][2]) {
...
__device__ void LoadKangaroo(GPU_U64_PTR a,uint32_t stride,uint64_t px[4],uint64_t py[4]) {
...
__device__ void LoadKangaroo(GPU_U64_PTR a,uint32_t stride,uint64_t px[4]) {
...
#ifdef USE_SYMMETRY
__device__ void StoreKangaroos(GPU_U64_PTR a,uint64_t px[GPU_GRP_SIZE][4],uint64_t py[GPU_GRP_SIZE][4],uint64_t dist[GPU_GRP_SIZE][2],GPU_U64_PTR jumps) {
#else
__device__ void StoreKangaroos(GPU_U64_PTR  a,uint64_t px[GPU_GRP_SIZE][4],uint64_t py[GPU_GRP_SIZE][4],uint64_t dist[GPU_GRP_SIZE][2]) {
#endif
...
__device__ void StoreKangaroo(GPU_U64_PTR a,uint32_t stride,uint64_t px[4],uint64_t py[4]) {
...
__device__ void StoreDists(GPU_U64_PTR a,uint64_t dist[GPU_GRP_SIZE][2]) {
...
__device__ void IMult(GPU_U64_PTR r,GPU_U64_PTR a,gpu_int64 b) {
...
__device__ uint64_t IMultC(GPU_U64_PTR r,GPU_U64_PTR a,gpu_int64 b) {
...
__device__ void MulP(GPU_U64_PTR r,uint64_t a) {
...
__device__ void ModNeg256(GPU_U64_PTR r,GPU_U64_PTR a) {
...
__device__ void ModNeg256(GPU_U64_PTR r) {
...
__device__ void ModSub256(GPU_U64_PTR r,GPU_U64_PTR a,GPU_U64_PTR b) {
...
__device__ void ModSub256(GPU_U64_PTR r,GPU_U64_PTR b) {
...
__device__ bool ModPositive256(GPU_U64_PTR r) {
...
__device__ void ModNeg256Order(GPU_U64_PTR r) {
...
__device__ __forceinline__ uint32_t ctz64(uint64_t x) {
#ifdef __METAL_VERSION__
  return x == 0ULL ? 64u : static_cast<uint32_t>(metal::ctz(x));
#else
  uint32_t n;
  asm("{\n\t"
    " .reg .u64 tmp;\n\t"
...
__device__ void _DivStep62(uint64_t u[5],uint64_t v[5],
                           GPU_I32_PTR pos,
                           GPU_I64_PTR uu,GPU_I64_PTR uv,
                           GPU_I64_PTR vu,GPU_I64_PTR vv) {
...
__device__ void MatrixVecMulHalf(uint64_t dest[5],uint64_t u[5],uint64_t v[5],gpu_int64 _11,gpu_int64 _12,GPU_U64_PTR carry) {
...
__device__ void MatrixVecMul(uint64_t u[5],uint64_t v[5],gpu_int64 _11,gpu_int64 _12,gpu_int64 _21,gpu_int64 _22) {
...
__device__ __noinline__ void _ModInv(GPU_U64_PTR R) {
