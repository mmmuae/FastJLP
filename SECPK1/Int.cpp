...
std::string Int::GetBlockStr() {
	...
	tmp[0] = 0;
        for (int i = NB32BLOCK-3; i>=0 ; i--) {
          std::snprintf(bStr, sizeof(bStr), "%08X", bits[i]);
          strcat(tmp, bStr);
          if(i!=0) strcat(tmp, " ");
        }
...
std::string Int::GetC64Str(int nbDigit) {
  ...
  for (int i = 0; i< nbDigit; i++) {
    if (bits64[i] != 0) {
#ifdef WIN64
      std::snprintf(bStr, sizeof(bStr), "0x%016I64XULL", bits64[i]);
#else
      std::snprintf(bStr, sizeof(bStr), "0x%" PRIx64  "ULL", bits64[i]);
#endif
    } else {
      std::snprintf(bStr, sizeof(bStr), "0ULL");
    }
    strcat(tmp, bStr);
...
#else
  printf("ModInv() cycles : n/a on this architecture\n");
#endif
