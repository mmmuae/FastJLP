...
// Missing intrinsics for non-MSVC x86 builds
...
static inline int64_t _mul128(int64_t a, int64_t b, int64_t *h) {
  ...
  *h = static_cast<int64_t>(rhi);
  return static_cast<int64_t>(rlo);
}
...
static inline uint64_t _umul128(uint64_t a, uint64_t b, uint64_t *h) {
  ...
  *h = static_cast<uint64_t>(res >> 64);
  return static_cast<uint64_t>(res);
}

static inline int64_t _mul128(int64_t a, int64_t b, int64_t *h) {
  __int128 res = static_cast<__int128>(a) * static_cast<__int128>(b);
  *h = static_cast<int64_t>(res >> 64);
  return static_cast<int64_t>(res);
}

static inline uint64_t _udiv128(uint64_t hi, uint64_t lo, uint64_t d, uint64_t *r) {
  unsigned __int128 numerator = (static_cast<unsigned __int128>(hi) << 64) | static_cast<unsigned __int128>(lo);
  uint64_t q = static_cast<uint64_t>(numerator / d);
  *r = static_cast<uint64_t>(numerator % d);
  return q;
}
...
static inline unsigned char subborrow_u64(unsigned char borrowIn, uint64_t a, uint64_t b, uint64_t *out) {
  unsigned __int128 subtrahend = static_cast<unsigned __int128>(b) + static_cast<unsigned __int128>(borrowIn);
  unsigned __int128 result = static_cast<unsigned __int128>(a) - subtrahend;
  *out = static_cast<uint64_t>(result);
  return static_cast<unsigned char>(result > static_cast<unsigned __int128>(a));
}

static inline unsigned char addcarry_u64(unsigned char carryIn, uint64_t a, uint64_t b, uint64_t *out) {
  unsigned __int128 result = static_cast<unsigned __int128>(a) + static_cast<unsigned __int128>(b) + static_cast<unsigned __int128>(carryIn);
  *out = static_cast<uint64_t>(result);
  return static_cast<unsigned char>(result >> 64);
}

#define _subborrow_u64(a,b,c,d) subborrow_u64(a,b,c,d)
#define _addcarry_u64(a,b,c,d) addcarry_u64(a,b,c,d)
#define _byteswap_uint64 __builtin_bswap64
#define LZC(x) __builtin_clzll(x)
#define TZC(x) __builtin_ctzll(x)
