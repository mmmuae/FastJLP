...
The legacy `Makefile` still works, but Apple Silicon CPUs deliver substantially better throughput when the project is compiled with CMake/Ninja using Clang’s native ARM64 optimizations. The repository now contains a standalone `CMakeLists.txt`, so you can build everything directly from the `Kangaroo256` directory:
...
The configuration automatically picks the `Release` profile, enforces C++17, and pins the target architecture to `arm64` when you invoke it on macOS. Passing `-DCMAKE_BUILD_TYPE=RelWithDebInfo` or other standard CMake settings still works if you need symbols for profiling.
